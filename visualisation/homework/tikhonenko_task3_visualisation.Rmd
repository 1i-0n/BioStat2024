...
t_test_result <- birthdata %>%
  ...
  mutate(
    ...
    y.position = max(birthdata$lowph, na.rm = TRUE) + 0.5  # Позиция p-value на графике
  )
?stat_pvalue_manual

# Визуализация
ggboxplot(birthdata, x = "inout", y = "lowph", add = "jitter") +
  stat_pvalue_manual(t_test_result, label = "p_value", y.position = "y.position") +
  labs(
...
test_result <- t_test_result  # Или wilcox_test_result

# Визуализация
ggboxplot(birthdata, x = "inout", y = "lowph", add = "jitter") +
  stat_pvalue_manual(test_result, label = "p", y.position = max(birthdata$lowph, na.rm = TRUE)) +
  labs(
    title = "Сравнение значений lowph между группами inout",
    x = "Группа (inout)",
    y = "lowph"
